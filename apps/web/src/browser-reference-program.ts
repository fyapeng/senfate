...
export async function loadBrowserReferenceProgram(): Promise<readonly CompiledReferenceRecord[]> {
  programPromise ??= (async () => {
    ...
    if (payload.schema !== "senfate-curated-runtime-program.v1" || !Array.isArray(payload.records) || payload.records.length !== 4_330 || payload.records.some(record => !record || typeof record !== "object" || (record as {disposition?:unknown}).disposition !== "executable")) {
