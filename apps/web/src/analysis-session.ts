...
function request(value:unknown):value is ApiAnalysisRequest{const keys=["schemaVersion","targetYear","locationId","localDateTime","sex","modelId","modelOverrides","schoolId","exactCoordinates","disambiguation","clockUncertaintySeconds","periodCount"];return record(value)&&Object.keys(value).every(key=>keys.includes(key))&&value.schemaVersion===ANALYSIS_REQUEST_SCHEMA&&Number.isInteger(value.targetYear)&&Number(value.targetYear)>=1850&&Number(value.targetYear)<=2200&&Number.isInteger(value.locationId)&&record(value.localDateTime)&&Number.isInteger(value.localDateTime.year)&&Number.isInteger(value.localDateTime.month)&&Number.isInteger(value.localDateTime.day)&&Number.isInteger(value.localDateTime.hour)&&Number.isInteger(value.localDateTime.minute)&&(value.sex==="female"||value.sex==="male")&&(value.modelId===undefined||value.modelId==="transparent-baseline"||value.modelId==="month-command"||value.modelId==="climate-priority")&&(value.schoolId===undefined||value.schoolId==="integrated-classical"||value.schoolId==="ziping-pattern"||value.schoolId==="climate-priority")&&(value.disambiguation===undefined||value.disambiguation==="earlier"||value.disambiguation==="later"||value.disambiguation==="reject")&&(value.modelOverrides===undefined||record(value.modelOverrides))&&(value.exactCoordinates===undefined||record(value.exactCoordinates))}
...
export function parseAnalysisSession(raw:string|null):AnalysisSessionSnapshot|undefined{
  ...
  if(!record(value)||value.version!==2||typeof value.date!=="string"||!/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/u.test(value.date)||typeof value.time!=="string"||!/^[0-9]{2}:[0-9]{2}$/u.test(value.time)||!Number.isInteger(value.targetYear)||Number(value.targetYear)<1850||Number(value.targetYear)>2200||(value.sex!=="female"&&value.sex!=="male")||!finite(value.clockUncertaintySeconds)||typeof value.useExactCoordinates!=="boolean"||typeof value.latitude!=="string"||typeof value.longitude!=="string"||typeof value.coordinateUncertaintyMeters!=="string"||(value.disambiguation!=="earlier"&&value.disambiguation!=="later"&&value.disambiguation!=="reject")||typeof value.query!=="string"||!ANALYSIS_TABS.includes(value.activeTab as AnalysisTab))return undefined;
