...
export function parseAnalysisSession(raw:string|null):AnalysisSessionSnapshot|undefined{
  ...
  if(!record(value)||value.version!==2||typeof value.date!=="string"||!/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/u.test(value.date)||typeof value.time!=="string"||!/^[0-9]{2}:[0-9]{2}$/u.test(value.time)||!Number.isInteger(value.targetYear)||Number(value.targetYear)<1850||Number(value.targetYear)>2150||(value.sex!=="female"&&value.sex!=="male")||!finite(value.clockUncertaintySeconds)||typeof value.useExactCoordinates!=="boolean"||typeof value.latitude!=="string"||typeof value.longitude!=="string"||typeof value.coordinateUncertaintyMeters!=="string"||(value.disambiguation!=="earlier"&&value.disambiguation!=="later"&&value.disambiguation!=="reject")||typeof value.query!=="string"||!ANALYSIS_TABS.includes(value.activeTab as AnalysisTab))return undefined;
