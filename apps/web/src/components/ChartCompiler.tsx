...
export default function ChartCompiler() {
  ...
  async function compile(form: HTMLFormElement) {
    ...
    let result: any;
    try {
      result = await compileChartInBrowser({ year, month, day, hour, minute, timeZone: String(useManualLocation ? data.get("timeZone") : city.timeZone), longitudeDegrees: useManualLocation ? Number(data.get("longitude")) : city.longitude, latitudeDegrees: useManualLocation ? Number(data.get("latitude")) : city.latitude, sex });
    } catch (cause) { setError(cause instanceof Error ? cause.message : "本地排盘失败。"); return; }
