...
import { compileChartInBrowser } from "../lib/calendar-client";
import { analyzeChartInBrowser } from "../lib/analysis-client";
...
export default function ChartCompiler() {
  ...
  async function compile(form: HTMLFormElement) {
    ...
    const sex = String(data.get("sex")) as "female" | "male";
    const city = locations.find(item => item.code === cityCode) ?? defaultLocation;
    const useManualLocation = data.get("manualLocation") === "on";
    setCompiling(true);
    setError("");
    try {
      const result = await compileChartInBrowser({ year, month, day, hour, minute, timeZone: String(useManualLocation ? data.get("timeZone") : city.timeZone), longitudeDegrees: useManualLocation ? Number(data.get("longitude")) : city.longitude, latitudeDegrees: useManualLocation ? Number(data.get("latitude")) : city.latitude, sex });
      const enriched = { ...result, city: city.label, sex };
      setCompiled(enriched);
    ...
    } catch (cause) { setError(cause instanceof Error ? cause.message : "本地排盘失败。"); }
  ...
  async function run() {
    ...
    try {
      ...
      const result = await analyzeChartInBrowser(compiled, compiled.sex || "female", targetYear);
      setAnalysis(result);
      sessionStorage.setItem("senfate.analysis", JSON.stringify(result));
      window.location.assign(`${import.meta.env.BASE_URL}`);
    } catch (cause) { setError(cause instanceof Error ? cause.message : "本地规则计算失败，请稍后重试。"); }
