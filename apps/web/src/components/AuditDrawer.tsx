...
export default function AuditDrawer({ label, year, annual, luck, verdict, audit }: any) {
  ...
  return <aside className="detail-drawer">
    ...
    <div className="insight-card"><span className="tag">本命原局</span><h2>{verdict?.headline || "本派暂未形成原局摘要"}</h2><p>{verdict?.summary}</p></div>
    <details open className="audit-fold"><summary><b>1</b><span>原局结论</span></summary><p>{verdict?.natal_points?.map((point: any) => `${point.label}：${point.text}`).join(" ") || "本派没有形成原局结论。"}</p></details>
    <details open className="audit-fold"><summary><b>2</b><span>岁运判定 · {luck ? `${luck.stem}${luck.branch}` : "—"}</span></summary><p>{verdict?.timing_focus || "本派没有形成岁运判定。"}</p></details>
