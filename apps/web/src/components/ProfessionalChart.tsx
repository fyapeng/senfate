...
function StemBranch({ stem, branch, large = false, stemMark, branchMark }: { stem: string; branch: string; large?: boolean; stemMark?: "日主"; branchMark?: "月令" | "日坐" }) {
  return <div className={`sb ${large ? "large" : ""}`}><b className={elementClass[stemElements[stem]]}>{stem}{stemMark && <sup className="pillar-mark day-master">{stemMark}</sup>}</b><strong className={elementClass[branchElements[branch]]}>{branch}{branchMark && <sup className={`pillar-mark ${branchMark === "月令" ? "month-command" : "day-seat"}`}>{branchMark}</sup>}</strong></div>;
}
