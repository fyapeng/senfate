...
export function ModelConfigurator() {
  ...
  return <div className="configurator"><aside className="preset-panel"><div className="panel-heading"><div><span className="step-label">MODEL PRESETS</span><h2>模型预设</h2></div><span className="version-pill">v0.1</span></div>{presets.map(x=><button type="button" className={selected===x.id?"selected":""} onClick={()=>choose(x.id)} key={x.id}><i></i><span><strong>{x.name}</strong><small>{x.desc}</small></span></button>)}<div className="preset-note"><span>配置边界</span><p>当前控件用于产品交互验收，尚不驱动正式推断结果。</p></div></aside><section className="parameter-panel"><div className="parameter-header"><div><span>当前配置</span><h2>{preset.name}</h2><p>{preset.desc}</p></div><div><button type="button">比较模型</button><button type="button" className="dark">导出 JSON</button></div></div><div className="parameter-tabs">{groups.map((x,i)=><button type="button" className={group===i?"active":""} onClick={()=>setGroup(i)} key={x.title}>{x.title}</button>)}</div><div className="parameter-list">{groups[group].items.map((item,i)=><label key={item}><div><strong>{item}</strong><small>{["控制该结构在模型中的相对作用","用于显示敏感性，并非经验概率","变更后要求当前阶段重新求值","随模型版本写入计算证书"][i]}</small></div><input aria-label={item} type="range" min="0" max="100" value={values[i]} onChange={e=>setValues(v=>v.map((x,j)=>j===i?Number(e.target.value):x))}/><output>{values[i]}</output></label>)}</div><div className="config-footer"><div><span>配置摘要</span><code>senfate-model.{selected}.v0.1</code></div><p>修改任何参数后，原局、大运与流年状态都必须完整重算。</p></div></section></div>;
}
