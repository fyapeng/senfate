...
const errorLabels: Readonly<Record<string, string>> = {
  ...
  "outside-ephemeris-range": "当前认证节气表支持 1850—2200 年。",
};
    ...
    <div className="workbench live-workbench">
      <aside className="input-panel">
        ...
        <form onSubmit={calculate}>
          <label>出生日期<input type="date" min="1850-01-01" max="2200-12-31" value={date} onChange={(event) => setDate(event.target.value)} required /></label>
...
function LifeTrajectoryResult({result,loading,selectingYear,onSelectYear,onOpenAnnual}:{result:ApiAnalysisResponse;loading:boolean;selectingYear:number|undefined;onSelectYear:(year:number)=>void;onOpenAnnual:()=>void}){
  const trajectory=result.annualTrajectory;const points=trajectory.points;const targetIndex=Math.max(0,points.findIndex(point=>point.year===result.annual.targetYear));const initialSize=points.length;
  const [viewSize,setViewSize]=useState(initialSize);const [viewStart,setViewStart]=useState(0);const drag=useRef<Readonly<{clientX:number;start:number}>|undefined>(undefined);
  ...
  const visiblePoints=points.slice(viewStart,viewStart+viewSize);const width=1040;const height=440;const plotTop=42;const plotHeight=330;const x=(index:number)=>58+(visiblePoints.length<=1?0:index*(width-94)/(visiblePoints.length-1));
  ...
  const visibleTickEvery=Math.max(1,Math.ceil(visiblePoints.length/10));
  const candleSpacing=visiblePoints.length>1?(width-94)/(visiblePoints.length-1):0;
  const candleWidth=Math.max(5,Math.min(16,candleSpacing*0.62));
  ...
  function resetView(){const next=points.length;setViewSize(next);setViewStart(0)}
  ...
  return <div className="life-trajectory-result">
    ...
    <div className="trajectory-toolbar" aria-label="轨迹视图控制"><div><strong>缩放</strong><button type="button" onClick={()=>zoomAt("out")} aria-label="缩小">−</button><button type="button" onClick={()=>zoomAt("in")} aria-label="放大">＋</button><button type="button" onClick={resetView}>显示全部</button><small className="zoom-indicator">当前 {viewSize}/{points.length} 年</small></div><div><span>{visiblePoints[0]?.year}—{visiblePoints.at(-1)?.year}</span></div></div>
    <div className="trajectory-chart interactive" role="img" aria-label={`${trajectory.startYear}年至${trajectory.endYear}人生状态轨迹，可滚轮缩放并拖动平移`} onWheel={event=>{event.preventDefault();const box=event.currentTarget.getBoundingClientRect();zoomAt(event.deltaY<0?"in":"out",Math.max(0,Math.min(1,(event.clientX-box.left)/box.width)))}} onPointerDown={event=>{drag.current={clientX:event.clientX,start:viewStart};event.currentTarget.setPointerCapture(event.pointerId)}} onPointerMove={event=>{if(!drag.current||viewSize>=points.length)return;const pixelsPerPoint=event.currentTarget.clientWidth/Math.max(1,viewSize-1);const delta=Math.round((drag.current.clientX-event.clientX)/pixelsPerPoint);setViewStart(Math.max(0,Math.min(points.length-viewSize,drag.current.start+delta)))}} onPointerUp={event=>{drag.current=undefined;event.currentTarget.releasePointerCapture(event.pointerId)}} onPointerCancel={()=>{drag.current=undefined}}><svg viewBox={`0 0 ${width} ${height}`} width="100%" height={height}>
      ...
      {visiblePoints.map((point,index)=>{if(point.status!=="stable"){const isPending=point.failureCode==="trajectory-not-loaded";const canOpen=isPending&&selectingYear===undefined;return <g className={`${isPending?"trajectory-pending":"trajectory-gap"} ${point.year===result.annual.targetYear?"selected":""} ${canOpen?"selectable":""}`} key={point.year} role={canOpen?"button":undefined} tabIndex={canOpen?0:undefined} aria-label={canOpen?`查看 ${point.year} 年完整分析`:undefined} onClick={()=>{if(canOpen)onSelectYear(point.year)}} onKeyDown={event=>{if(canOpen&&(event.key==="Enter"||event.key===" ")){event.preventDefault();onSelectYear(point.year)}}}><line x1={x(index)} x2={x(index)} y1={plotTop} y2={plotTop+plotHeight}/><title>{point.year} · {isPending?"年度轨迹正在生成，可先查看完整分析":"年度不可用"}</title></g>}const candle=point.monthlyCandle;const direction=candle.status==="stable"&&candle.close<candle.open?"negative":"positive";const canOpen=selectingYear===undefined;return <g className={`trajectory-candle ${direction} ${point.year===result.annual.targetYear?"selected":""} ${canOpen?"selectable":""}`} key={point.year} role="button" tabIndex={canOpen?0:undefined} aria-label={`查看 ${point.year} 年完整分析`} onClick={()=>{if(canOpen)onSelectYear(point.year)}} onKeyDown={event=>{if(canOpen&&(event.key==="Enter"||event.key===" ")){event.preventDefault();onSelectYear(point.year)}}}>{point.year===result.annual.targetYear&&<line className="selected-year-guide" x1={x(index)} x2={x(index)} y1={plotTop-12} y2={plotTop+plotHeight+10}/>} {candle.status==="stable"?<><line className="candle-wick" x1={x(index)} x2={x(index)} y1={y(candle.high)} y2={y(candle.low)} strokeWidth={Math.max(1,candleWidth*0.16)}/><rect className="candle-body" x={x(index)-candleWidth/2} y={Math.min(y(candle.open),y(candle.close))} width={candleWidth} height={Math.max(3,Math.abs(y(candle.open)-y(candle.close)))} rx={Math.min(2,candleWidth*0.18)}/></>:<line className="monthly-gap" x1={x(index)} x2={x(index)} y1={y(.08)} y2={y(-.08)}/>}<circle cx={x(index)} cy={y(point.normalizedTopicIndex)} r={point.year===result.annual.targetYear?4:2.5}/>{point.specialStateCodes.length>0&&<path d={`M ${x(index)-candleWidth/2} ${plotTop-4} L ${x(index)+candleWidth/2} ${plotTop-4} L ${x(index)} ${plotTop+5} Z`}/>}<title>{point.year} · {ganZhi(point.luckPillar)}运 / {ganZhi(point.annualPillar)}年 · 年度方向 {decimal(point.normalizedTopicIndex,3)} · {candle.status==="stable"?`流月 开 ${decimal(candle.open,3)} / 高 ${decimal(candle.high,3)} / 低 ${decimal(candle.low,3)} / 收 ${decimal(candle.close,3)}`:"流月序列不可用"}{point.specialStateCodes.length?` · ${point.specialStateCodes.map(code=>specialStateLabels[code]??"特殊状态").join("、")}`:""}</title></g>})}
      {visiblePoints.map((point,index)=>index%visibleTickEvery===0||index===visiblePoints.length-1?<text className="trajectory-year" x={x(index)} y={height-32} textAnchor="middle" key={`year-${point.year}`}>{point.year}</text>:null)}
    </svg></div>
    <div className="trajectory-navigator"><input type="range" min="0" max={Math.max(0,points.length-viewSize)} value={viewStart} disabled={viewSize>=points.length} onChange={event=>setViewStart(Number(event.target.value))} aria-label="移动人生轨迹可视区间"/><span>滚轮放大缩小 · 放大后拖动选择区间 · 点击 K 线查看年度</span></div>
