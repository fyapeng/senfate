...
async function compileBundledProgram():Promise<readonly CompiledReferenceRecord[]> {
  const bytes=new Uint8Array(programBytes);const gzip=bytes[0]===0x1f&&bytes[1]===0x8b;
  const payload=(gzip?await new Response(new Blob([bytes]).stream().pipeThrough(new DecompressionStream("gzip"))).json():JSON.parse(new TextDecoder().decode(bytes))) as Readonly<{schema?:string;records?:unknown}>;
  if(payload.schema!=="senfate-curated-runtime-program.v1"||!Array.isArray(payload.records)||payload.records.length!==4_330||payload.records.some(record=>!record||typeof record!=="object"||(record as {disposition?:unknown}).disposition!=="executable"))throw new Error("reference-program-integrity-error");
