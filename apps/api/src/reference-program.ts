...
async function compileBundledProgram():Promise<readonly CompiledReferenceRecord[]> {
  ...
  if(payload.schema!=="senfate-curated-runtime-program.v1"||!Array.isArray(payload.records)||payload.records.length!==4_332||payload.records.some(record=>!record||typeof record!=="object"||(record as {disposition?:unknown}).disposition!=="executable"))throw new Error("reference-program-integrity-error");
