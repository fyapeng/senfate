...
export async function handleRequest(request: Request, locations?: LocationStore,program:ReferenceProgramStore=bundledReferenceProgram): Promise<Response> {
  ...
  if(trajectoryPath){if(request.method!=="POST")return error(requestId,405,"method-not-allowed","Use POST for the trajectory fallback");const startYear=Number(url.searchParams.get("startYear")),endYear=Number(url.searchParams.get("endYear"));if(!Number.isInteger(startYear)||!Number.isInteger(endYear)||startYear<PUBLIC_START_YEAR||endYear>PUBLIC_END_YEAR||endYear!==startYear)return error(requestId,400,"invalid-trajectory-range","The server fallback accepts exactly one year");return calculate(request,requestId,locations,true,program,{startYear,endYear})}
  if (analysisPath) {
  ...
  if (pathname === `${API_PREFIX}/meta` || pathname === "/meta") {
    const body: ApiMetaResponse = {
      ...
      corpus: { version: "runtime.v1", records: 4_332, books: 7 }, calculationStatus: "curated-traditional-rule-runtime",
    };
    return json(body);
  }
