...
export async function handleRequest(request: Request, locations?: LocationStore): Promise<Response> {
  ...
  const { pathname } = url;
  if (request.method !== "GET") return error(requestId, 405, "method-not-allowed", "Only GET is available during kernel reconstruction");
  ...
  if (pathname === `${API_PREFIX}/meta` || pathname === "/meta") {
    const body: ApiMetaResponse = {
      ...
      corpus: { version: "4.0", records: 37_231, families: 11_306, books: 7 }, calculationStatus: "kernel-rebuild",
    };
    return json(body);
  }
