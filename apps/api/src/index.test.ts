...
describe("SenFate API", () => {
  it("reports the canonical corpus baseline", async () => {
    ...
    await expect(response.json()).resolves.toMatchObject({ schemaVersion: "senfate-api-meta.v8", calculationStatus: "curated-traditional-rule-runtime", corpus: { version:"runtime.v1",records: 4_332, books: 7 } });
  });
  ...
  it("returns a stable natal structure analysis without changing the calendar contract", async () => {
    const response = await handleRequest(new Request("https://example.test/senfate/api/v1/analysis/calculate", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ schemaVersion: "senfate-analysis-request.v3", targetYear:2026,locationId: beijing.id, localDateTime: { year: 2000, month: 2, day: 10, hour: 12, minute: 0 }, sex: "male" }),
    }), store,program);
    expect(response.status).toBe(200);
    ...
    expect(body).toMatchObject({
      ...
      interpretation: { schema: "senfate-interpretive-model.v2", pattern:{schema:"senfate-pattern-projection.v3"}, climate: { schema: "senfate-climate-coordinate.v1" } },
  ...
  it("applies only bounded public model overrides and certifies the effective configuration",async()=>{
    ...
    const response=await handleRequest(request({temporalLayers:{annual:1.5,month:1.25},topics:{domainWeights:{career:2}}}),store,program);expect(response.status).toBe(200);const body=await response.json() as ApiAnalysisResponse;expect(body.modelConfiguration).toMatchObject({customized:true,overrideCount:3,overrides:{temporalLayers:{annual:1.5,month:1.25},topics:{domainWeights:{career:2}}}});expect(body.modelConfiguration.overrideFingerprint).toMatch(/^[0-9a-f]{8}$/u);expect(body.calendar.model.version).toContain(body.modelConfiguration.overrideFingerprint);expect(body.annual.topics.contribution.atoms.career).toBe(6);
