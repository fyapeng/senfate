...
- 37,231-record corpus integrity audit;
- canonical stems, branches, elements, yin-yang and hidden stems;
- sexagenary-cycle and ten-god derivation;
- stem/branch relation candidate detection;
- civil-time, longitude and equation-of-time normalization;
- four-pillar and major-luck calculation from a certified solar-term window;
- interval-based failure closure at solar-term, day and hour boundaries;
- finite signed measures and certified functional contracts;
- generic normal-form evaluator with `cycle` and `limit-reached` closure;
...
- element- and ten-god-colored chart notation plus an auditable life-trajectory K-line whose open/high/low/close values come from twelve flow-month recomputations; a browser Web Worker loads the versioned rule corpus once, streams locally evaluated years into the chart, and preserves explicit failures as gaps.
...
- sentence-level candidate extraction for the narrow `日主 + 月令 + 明确结果` form is available through `pnpm --filter @senfate/rules extract:semantic-candidates`; this command only proposes review items and never changes the public runtime by itself;
