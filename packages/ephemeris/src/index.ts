...
export function compileCertifiedBaziCalendar(input:ZonedCivilBirthInput,model:SenFateModelProfile,sex:"female"|"male",periodCount=8):ClosedResult<CertifiedBaziCalendarResult,CertifiedCalendarFailure>{
  const zoned=resolveZonedCivilBirth(input);if(!zoned.ok)return zoned;const terms=solarTermWindowAt(zoned.value.selected.utcMs);if(!terms.ok)return terms;const calendar=compileBaziCalendar(zoned.value.civilBirth,calendarProfileFromModel(model),terms.value,sex,periodCount);if(!calendar.ok)return calendar;
  return{ok:true,value:{schema:"senfate-certified-bazi-calendar.v1",zonedBirth:zoned.value,solarTermWindow:terms.value,calendar:calendar.value},certificate:{functional:"calendar.certified-bazi",model:`${model.id}@${model.version}`,tzdbVersion:TZDB_VERSION,ephemerisDigest:EPHEMERIS_MANIFEST.sha256,location:{timeZone:input.timeZone,longitude:input.longitudeDegrees,latitude:input.latitudeDegrees},upstream:{timeZone:zoned.certificate,solarTerms:terms.certificate,calendar:calendar.certificate}}};
}
