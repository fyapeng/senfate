...
export function solarTermWindowAt(utcMs:number):ClosedResult<SolarTermWindow,EphemerisFailure>{
  const certificate={functional:"ephemeris.solar-term-window",ephemeris:`JPL-DE441@${EPHEMERIS_MANIFEST.generatedAt}`,digest:EPHEMERIS_MANIFEST.sha256};if(!Number.isFinite(utcMs)||!validTable())return{ok:false,code:"invalid-ephemeris-table",reason:"Solar-term table failed integrity validation",certificate};
