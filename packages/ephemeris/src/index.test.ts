...
describe("pinned JPL solar-term table",()=>{
  ...
  it("calculates pillars, luck direction and luck start from the pinned table",()=>{const result=compileCertifiedBaziCalendar({year:2000,month:2,day:10,hour:12,minute:0,timeZone:"Asia/Shanghai",longitudeDegrees:121.4737,latitudeDegrees:31.2304},TRANSPARENT_BASELINE_MODEL,"male","test-tzdb",2);expect(result.ok).toBe(true);if(!result.ok)return;expect(result.value.calendar.pillars.year).toMatchObject({stem:"庚",branch:"辰"});expect(result.value.calendar.direction).toBe("forward");expect(result.value.calendar.majorLuck).toHaveLength(2);expect(result.value.solarTermWindow.ephemerisDigest).toBe(EPHEMERIS_MANIFEST.sha256)});
});
