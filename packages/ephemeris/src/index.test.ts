...
describe("pinned JPL solar-term table",()=>{
  ...
  it("covers the full public range through the end of 2200",()=>{const result=solarTermWindowAt(Date.UTC(2200,11,31,12));expect(result.ok).toBe(true);if(!result.ok)return;expect(result.value.baziYear).toBe(2200);expect(result.value.nextJieUtcMs).toBeGreaterThan(Date.UTC(2201,0,1))});
