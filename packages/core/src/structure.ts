...
export function evaluateDayMasterStrength(pillars:FourPillarState,model:SenFateModelProfile):ClosedResult<StrengthEvaluation>{
  const measureResult=elementMeasureFunctional(elementMeasureProfileFromModel(model,pillars.month.branch)).evaluate(pillars);if(!measureResult.ok)return measureResult;const roots=materializeRootExposure(pillars,model);const dayElement=STEM_DEFINITIONS[pillars.day.stem].element;const resourceElement=inverse(GENERATES,dayElement);const outputElement=GENERATES[dayElement];const wealthElement=CONTROLS[dayElement];const officerElement=inverse(CONTROLS,dayElement);const atoms=measureResult.value.measure.atoms;
