...
export function resolveAnnualContext(targetYear: number, boundaryUtcMs:number,periods: readonly MajorLuckPeriod[]): ClosedResult<AnnualContext, AnnualContextFailure> {
  ...
  if (!Number.isInteger(targetYear) || targetYear < 1850 || targetYear > 2100||!Number.isFinite(boundaryUtcMs)) return { ok: false, code: "invalid-target-year", reason: "Target year and certified Lichun instant must be in the 1850–2100 range", certificate };
