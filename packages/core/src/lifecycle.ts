...
export type TemporalLayer = CalculationPhase;
...
export interface DynamicChartState {
  ...
  readonly luck?:GanZhi; readonly annual?:GanZhi; readonly pillars:readonly LayeredPillar[];
...
export function materializeDynamicChartState(input:Readonly<{natal:FourPillarState;luck?:GanZhi;annual?:GanZhi}>,model:SenFateModelProfile):ClosedResult<DynamicChartState,DynamicStateFailure>{
  ...
  if(input.annual&&!input.luck)return{ok:false,code:"missing-luck-layer",reason:"Annual state requires the enclosing major-luck layer",certificate};
  ...
  return{ok:true,value:{schema:"senfate-dynamic-chart-state.v1",phase,natal:input.natal,...(input.luck?{luck:input.luck}:{}),...(input.annual?{annual:input.annual}:{}),pillars,elementMeasure:measure,strength:{state,supportRatio,support,pressure},relationCandidates},certificate:{...certificate,layerWeights:model.temporalLayers,pillarIds:pillars.map(item=>item.id)}};
}
