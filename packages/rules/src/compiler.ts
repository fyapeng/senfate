...
export function compileReferenceCorpusData(input:unknown):ReferenceCompilationAudit {
  ...
  corpus.rules.forEach((bookRules,bookIndex)=>bookRules.forEach((raw,ruleIndex)=>{
    ...
    const elementTerms=terms.elements??[];const unboundElementConditionIndexes=conditions.map((condition,index)=>condition.operator==="element.state"&&!condition.subject?index:-1).filter(index=>index>=0);if(unboundElementConditionIndexes.length>0){if(elementTerms.length!==unboundElementConditionIndexes.length)unsupported=true;else{const subjects=new Map(unboundElementConditionIndexes.map((conditionIndex,index)=>[conditionIndex,elementTerms[index]!]));conditions=conditions.map((condition,index)=>condition.operator==="element.state"&&!condition.subject?{...condition,subject:subjects.get(index)!}:condition)}}
