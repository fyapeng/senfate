...
export type TopicEffectOperator="complete_or_transform"|"pressure"|"reveal"|"support"|"weaken_or_block";
...
export interface ActivatedEventEvidence { readonly operator:TopicEffectOperator; readonly domain:TopicDomain; readonly polarity:string }
export interface ActivatedReferenceSource { readonly recordId:string; readonly familyId:string; readonly bookId:string; readonly lineStart:number; readonly lineEnd:number; readonly domains:readonly TopicDomain[]; readonly polarity:string; readonly eventEvidence:readonly ActivatedEventEvidence[] }
export interface TopicEventHypothesis {
  readonly schema:"senfate-topic-event-hypothesis.v2"; readonly predicateId:string; readonly phase:ReferenceFeatureSnapshot["phase"];
  readonly domain:TopicDomain; readonly operator:TopicEffectOperator; readonly direction:"support"|"pressure"|"mixed"|"neutral";
  readonly contribution:number; readonly magnitude:number; readonly sourceCount:number; readonly familyCount:number; readonly bookCount:number;
  readonly evidenceStatus:"single-source"|"same-book-corroborated"|"cross-book-corroborated"|"mixed-evidence";
  readonly epistemicStatus:"traditional-model-hypothesis";
}
export interface ContributionCertificate {
  readonly schema:"senfate-topic-contribution-certificate.v3"; readonly phase:ReferenceFeatureSnapshot["phase"]; readonly model:string;
...
export function evaluateReferenceContributions(records:readonly CompiledReferenceRecord[],snapshot:ReferenceFeatureSnapshot,model:SenFateModelProfile):ContributionCertificate{
  ...
  for(const record of records){if(record.disposition!=="executable"||!scopeApplies(record,snapshot.phase))continue;evaluated++;const truth=kleeneAnd(record.conditions.map(condition=>evaluateReferenceCondition(condition,snapshot)));if(truth==="unknown"){unresolvedFamilies.push(record.familyId);continue}if(truth==="false"){inactive++;continue}activatedFamilies.push(record.familyId);const sourceDomains=new Set<TopicDomain>();const sourcePolarities=new Set<string>();const eventEvidence:ActivatedEventEvidence[]=[];for(const effect of record.effects){const domains=effect.domains.filter((domain):domain is TopicDomain=>TOPIC_DOMAINS.includes(domain as TopicDomain));for(const domain of domains){sourceDomains.add(domain);contributions[domain]+=polaritySign(effect.polarity)*(model.topics.domainWeights[domain]??0);if(isTopicEffectOperator(effect.operator))eventEvidence.push({operator:effect.operator,domain,polarity:effect.polarity})}sourcePolarities.add(effect.polarity)}activatedSources.push({recordId:record.recordId,familyId:record.familyId,bookId:record.bookId,lineStart:record.lineStart,lineEnd:record.lineEnd,domains:[...sourceDomains],polarity:sourcePolarities.size===1?[...sourcePolarities][0]!:"mixed",eventEvidence})}
  const groups=new Map<string,{domain:TopicDomain;operator:TopicEffectOperator;contribution:number;sources:Set<string>;families:Set<string>;books:Set<string>;polarities:Set<string>}>();
  for(const source of activatedSources)for(const evidence of source.eventEvidence){const key=`${evidence.domain}:${evidence.operator}`;let group=groups.get(key);if(!group){group={domain:evidence.domain,operator:evidence.operator,contribution:0,sources:new Set(),families:new Set(),books:new Set(),polarities:new Set()};groups.set(key,group)}group.contribution+=polaritySign(evidence.polarity)*(model.topics.domainWeights[evidence.domain]??0);group.sources.add(source.recordId);group.families.add(source.familyId);group.books.add(source.bookId);group.polarities.add(evidence.polarity)}
  const eventHypotheses:TopicEventHypothesis[]=[...groups.values()].map((group):TopicEventHypothesis=>{const conflict=group.polarities.has("support")&&group.polarities.has("pressure");const mixedDirection=conflict||group.polarities.has("mixed");const direction:TopicEventHypothesis["direction"]=group.contribution>0?"support":group.contribution<0?"pressure":mixedDirection?"mixed":"neutral";const evidenceStatus:TopicEventHypothesis["evidenceStatus"]=conflict?"mixed-evidence":group.books.size>1?"cross-book-corroborated":group.sources.size>1?"same-book-corroborated":"single-source";return{schema:"senfate-topic-event-hypothesis.v2",predicateId:`${snapshot.phase}:${group.domain}:${group.operator}`,phase:snapshot.phase,domain:group.domain,operator:group.operator,direction,contribution:group.contribution,magnitude:Math.abs(group.contribution),sourceCount:group.sources.size,familyCount:group.families.size,bookCount:group.books.size,evidenceStatus,epistemicStatus:"traditional-model-hypothesis"}}).sort((a,b)=>Math.abs(b.contribution)-Math.abs(a.contribution)||a.predicateId.localeCompare(b.predicateId));
  return{schema:"senfate-topic-contribution-certificate.v3",phase:snapshot.phase,model:`${model.id}@${model.version}`,program,evaluated,activated:activatedFamilies.length,inactive,unresolved:unresolvedFamilies.length,activatedFamilies,unresolvedFamilies,activatedSources,contribution:finiteSignedMeasure(TOPIC_DOMAINS,contributions),eventHypotheses};
}
