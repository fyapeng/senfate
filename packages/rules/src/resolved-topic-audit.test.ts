...
it("audits the complete resolved topic program",()=>{
  ...
  const ambiguousSymbolAbundance=audit.records.filter(record=>record.disposition==="deferred"&&record.reason==="condition-not-canonically-typed"&&record.conditions.some(condition=>condition.operator==="symbol.abundant"));
  expect(ambiguousSymbolAbundance).toHaveLength(0);expect(executable.some(record=>record.conditions.some(condition=>condition.operator==="element.state"&&condition.value==="abundant"))).toBe(true);
  ...
  expect(certificate.program).toEqual({total:4_329,executable:4_329,deferred:0,contested:0,evidence:0,fixture:0});
  ...
  expect(certificate.activatedSources.every(source=>source.conditions.length>0)).toBe(true);
  console.info(JSON.stringify({schema:"senfate-resolved-topic-feature-audit.v5",topicFunctions:executable.length,topicRules,eventPredicates:eventPredicates.size,deferred:audit.counts.deferred,deferredSemanticGaps:{ambiguousSymbolAbundance:ambiguousSymbolAbundance.length},contested:audit.counts.contested,canonicalFixture:{evaluated:certificate.evaluated,activated:certificate.activated,inactive:certificate.inactive,unresolved:certificate.unresolved,activatedSourceScopes,eventHypotheses:certificate.eventHypotheses.length,activatedConditionAtoms:certificate.activatedSources.reduce((sum,source)=>sum+source.conditions.length,0),evidenceStatus:Object.fromEntries([...new Set(certificate.eventHypotheses.map(item=>item.evidenceStatus))].map(status=>[status,certificate.eventHypotheses.filter(item=>item.evidenceStatus===status).length])),contribution:certificate.contribution.atoms}},null,2));
});
